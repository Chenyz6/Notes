...
答：当其他类对象作为本类成员，构造时候先构造类对象，再构造自身。
...
Person * const this       指针不可以修改

const Person * this       指针指向的值不可以修改
...
public:
	...
	//前置++
	MyInteger& operator++() {  //返回引用是为了第一个数据进行操作
		//先++
		m_Num++;
		//再返回
		return *this;
...
![赋值运算符重载 深拷贝](E:\BaiduSyncdisk\QT 笔记\赋值运算符重载 深拷贝.png)
